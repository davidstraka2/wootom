...
class PairMatcher {
     ...
     */
    constructor(beginRegex, endRegex) {
    ...
    findFirstMatch(source) {
        ...
        if (endRes === null)
            return;
        const matched = source.slice(beginRes.index, remainingSource.length > 0 ? -remainingSource.length : undefined);
        const inner = matched.slice(beginRes[0].length, -endRes[0].length);
        const groups = [...beginRes.slice(1), inner, ...endRes.slice(1)];
        return new match_1.Match(source, matched, beginRes.index, groups);
    }
    /**
     ...
     */
    getFirstRes(beginRes, endRes) {
        if (beginRes === null)
            return endRes;
        if (beginRes.index < endRes.index)
            return beginRes;
        return endRes;
    }
}
